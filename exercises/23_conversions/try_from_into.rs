...
impl TryFrom<(i16, i16, i16)> for Color {
	...
	fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
		let red_result: Result<u8, _> = tuple.0.try_into();
		let green_result: Result<u8, _> = tuple.1.try_into();
		let blue_result: Result<u8, _> = tuple.2.try_into();
...
impl TryFrom<[i16; 3]> for Color {
	...
	fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
		let red_result: Result<u8, _> = arr[0].try_into();
		let green_result: Result<u8, _> = arr[1].try_into();
		let blue_result: Result<u8, _> = arr[2].try_into();
		let red = match red_result {
			Ok(color) => color,
			Err(_) => return Err(IntoColorError::IntConversion),
		};
...
impl TryFrom<&[i16]> for Color {
	...
	fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
		...
		let red_result: Result<u8, _> = slice[0].try_into();
		let green_result: Result<u8, _> = slice[1].try_into();
		let blue_result: Result<u8, _> = slice[2].try_into();
		let red = match red_result {
			Ok(color) => color,
			Err(_) => return Err(IntoColorError::IntConversion),
		};
